...
| `ConfirmationExpiryJob` | Every 15 min | Reminds once (mail + push) an hour before the stored deadline; at the deadline, per location × date × slot, as many unconfirmed Won bookings expire as there are Waitlisted users (newest booking first; mail + push, slot to waitlist), the rest are kept and become Confirmed (mail + push, `AutoConfirmReason = kept_nobody_waiting`); past slot end the same rule applies silently (`kept_slot_ended`, no mails); closes Waitlisted past slot end as Lost |
...
- `Booking.AutoConfirmReason` records why the system confirmed a booking (`direct_assignment`, `waitlist_late_promotion`, `kept_nobody_waiting`, `kept_slot_ended`); null means the user clicked confirm. Exposed on the admin bookings list.
